...
func ComputeHash(key interface{}) (uint64, error) {
	...
	//bytes.NewBuffer(data)
 	binary.ReadUvarint(bytes.NewBuffer(data))
	/*err1 := binary.Read(bytes.NewReader(data), binary.LittleEndian, &pi)
	if err1 != nil {
        return 0, err1
    }
    var a, b, c uint64
    ...
    for i = 0; i < len(data)-12; {
		tt, _ := binary.ReadUvarint(bytes.NewBuffer(data[i+1] | data[i+2] << 8 | data[i+3] << 16 | data[i+4] << 24))
 		a += tt
        i += 4
        b += uint32(data[i+1] | data[i+2] << 8 | data[i+3] << 16 | data[i+4] << 24)
        i += 4
        c += uint32(data[i+1] | data[i+2] << 8 | data[i+3] << 16 | data[i+4] << 24)
